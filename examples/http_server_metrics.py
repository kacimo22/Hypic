...
sys.path.append("../src")
from metrics import QUIC_HANDSHAKE_DURATION

import Hypic
import wsproto
import wsproto.events
from Hypic.asyncio import QuicConnectionProtocol, serve
from Hypic.catalyst import ServerConfigLoader, CatalystTLSManager
from Hypic.h0.connection import H0_ALPN, H0Connection
from Hypic.h3.connection import H3_ALPN, H3Connection
...
from Hypic.h3.exceptions import NoAvailablePushIDError
from Hypic.quic.configuration import QuicConfiguration
from Hypic.quic.events import DatagramFrameReceived, ProtocolNegotiated, QuicEvent
from Hypic.quic.logger import QuicFileLogger
from Hypic.tls import SessionTicket
...
SERVER_NAME = "Hypic/" + Hypic.__version__
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--catalyst-config",
        type=str,
        default="catalyst_server_config.json",
        help="Catalyst server configuration file (default: catalyst_server_config.json)"
    )
    parser.add_argument(
        "--port",
        type=int,
    ...
    try:
        # Load Catalyst configuration
        config_file = args.catalyst_config if args.catalyst_config else "catalyst_server_config.json"
        server_config = ServerConfigLoader(config_file)
        catalyst_manager = CatalystTLSManager(config_file)

        # Store in configuration for TLS context
        configuration.catalyst_manager = catalyst_manager
        configuration.certificate_dir = str(server_config.get_certificates_dir())
        configuration.pq_kem=server_config.get_pq_kem().upper()

        # Determine if explicit certificate is provided
        if args.certificate and args.private_key:
            # Explicit certificate mode
            configuration.catalyst_mode = "explicit"
            configuration.load_cert_chain(args.certificate, args.private_key)

            logging.info(f" Certificate: {configuration.certificate}")

        else:
            # Dynamic selection mode
            configuration.catalyst_mode = "dynamic"
        logging.info(f"✅ Available certificates: {len(server_config.get_enabled_configs())}")

    except ImportError as e:
        logging.error(f"❌ ERROR: Catalyst negotiation module required but not found: {e}")
        sys.exit(1)
    except Exception as e:
        logging.error(f"❌ ERROR: Failed to initialize Catalyst system: {e}")
        sys.exit(1)
