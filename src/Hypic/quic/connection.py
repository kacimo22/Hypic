...
class QuicConnection:
    ...
    def __init__(
        ...
        *,
        ...
        self._enable_pq=configuration.enable_pq
        self._pq_kem=configuration.pq_kem
        ...
        self._ack_delay_handshake = 0.01  # 10ms instead of the default
        self._ack_delay_application = 0.025
    ...
    def connect(self, addr: NetworkAddress, now: float) -> None:
        ...
        #compute handshake timing (start handshake)
        if self._is_client:
    ...
    def get_timer(self) -> Optional[float]:
        ...
        if self._state not in END_STATES:
            # ack timer - use shorter delay for handshake packets
            for epoch, space in self._spaces.items():
                if space.ack_at is not None:
                    ack_delay = self._ack_delay_handshake if epoch in [tls.Epoch.INITIAL,
                                                                       tls.Epoch.HANDSHAKE] else self._ack_delay_application
                    if space.ack_at + ack_delay < timer_at:
                        timer_at = space.ack_at + ack_delay
    ...
    def _initialize(self, peer_cid: bytes) -> None:
        ...
        self.tls = tls.Context(
            ...
            is_client=self._is_client,
            enable_pq=self._enable_pq,
            VALID_TIME_RPQH=self.configuration.VALID_TIME_RPQH,
            pq_kem=self._pq_kem,
            logger=self._logger,
            ...
            catalyst_manager=self.configuration.catalyst_manager,
            catalyst_mode=self.configuration.catalyst_mode,
            certificate_dir=self._configuration.certificate_dir,
            certificate=self.configuration.certificate,
            certificate_chain=self.configuration.certificate_chain,
            certificate_private_key=self.configuration.private_key,
        )
        self.tls.handshake_extensions = [
            (
                tls.ExtensionType.QUIC_TRANSPORT_PARAMETERS,
...
# post-quantum modification
    def _is_ack_queue_empty(self, space: QuicPacketSpace) -> bool:
        """
        Check if the ack_queue is empty in a safe way.
        """
        try:
            # Try to check if the ack_queue is empty by checking if it has any ranges
            if hasattr(space.ack_queue, 'ranges'):
                return not bool(space.ack_queue.ranges)

            # Fallback: try to use the __bool__ method
            return not bool(space.ack_queue)
        except NotImplementedError:
            # If __bool__ is not implemented, assume it's not empty to be safe
            return False

    def _send_immediate_ack(self, epoch: tls.Epoch, now: float) -> None:
        """
        Immediately send an ACK for the specified epoch to avoid retransmissions.
        """
        space = self._spaces[epoch]

        # Check if there are any packets to acknowledge
        if not self._is_ack_queue_empty(space):
            # Force immediate ACK by setting ack_at to now
            space.ack_at = now

            # Set probe pending to ensure packets are sent immediately
            self._probe_pending = True

            # Log the immediate ACK request
            self._logger.debug(
                "Requesting immediate ACK for epoch %s",
                epoch
            )
    ...
    def _handle_crypto_frame(
        ...
        if event is not None:
            ...
            try:
                ...
                # Send immediate ACK for crypto frames to prevent retransmission
                space = self._spaces[context.epoch]

                # For handshake packets, request immediate ACK
                if context.epoch in (tls.Epoch.INITIAL, tls.Epoch.HANDSHAKE):
                    space.ack_at = context.time  # expire ack timer now
                    self._probe_pending = True  # force sending of packets

                    self._logger.debug(
                        "Immediate ACK requested for %s crypto frame",
                        context.epoch
                    )
            ...
            if not self._handshake_complete and self.tls.state in [
                ...
                # Record handshake completion time
                # if self._is_client:
                #     if self.handshake_start_time is not None:
                #         self.handshake_end_time = context.time
                #         self.handshake_duration = self.handshake_end_time - self.handshake_start_time
                #
                #
                #         # Log handshake duration
                #         self._logger.info(
                #             "Handshake completed in %.3f seconds", self.handshake_duration
                #         )
                #     else:
                #         self._logger.warning("Handshake completed but start time was not recorded")
