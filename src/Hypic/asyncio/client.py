...
async def connect(
    ...
    *,
    ...
    :class:`~Hypic.asyncio.QuicConnectionProtocol` which can be used to
    create streams.

    :func:`connect` also accepts the following optional arguments:

    * ``configuration`` is a :class:`~Hypic.quic.configuration.QuicConfiguration`
    ...
    * ``create_protocol`` allows customizing the :class:`~asyncio.Protocol` that
      ...
      arguments as :class:`~Hypic.asyncio.QuicConnectionProtocol` and returning
      an instance of :class:`~Hypic.asyncio.QuicConnectionProtocol` or a subclass.
    ...
    * ``wait_connected`` indicates whether the context manager should wait for the
      ...
      :class:`~Hypic.asyncio.QuicConnectionProtocol`. By default this is `True` but
