...
* Allow asyncio clients to make efficient use of 0-RTT by passing
  `wait_connected=False` to :meth:`~Hypic.asyncio.connect`.
...
* Make :class:`~Hypic.h3.connection.H3Connection`'s
  :meth:`~Hypic.h3.connection.H3Connection.send_datagram` and
  :meth:`~Hypic.h3.connection.H3Connection.send_push_promise` methods raise an
  :class:`~Hypic.h3.exceptions.InvalidStreamTypeError` exception if an
  invalid stream ID is specified.
* Improve the documentation for
  :class:`~Hypic.asyncio.QuicConnectionProtocol`'s
...
* Perform more extensive HTTP/3 header validation in
  :class:`~Hypic.h3.connection.H3Connection`.
* Fix exceptions when draining stream writers in the :doc:`asyncio API <asyncio>`.
* Set the :class:`~Hypic.quic.connection.QuicConnection` idle timer according to
  :rfc:`9000` section 10.1.
* Implement fairer stream scheduling in :class:`~Hypic.quic.connection.QuicConnection`
  to avoid head-of-line blocking.
* Only load `certifi`_ root certificates if none was specified in the
  :class:`~Hypic.quic.configuration.QuicConfiguration`.
